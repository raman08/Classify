...
exports.getDashboard = async (req, res) => {
	const cls = await User.findOne({ _id: req.user._id }).populate({
		path: 'classes',
		select: '_id name',
		populate: {
			path: 'faculty',
	...
	if (token) {
		jwt.verify(token, JWT_RESET_KEY, async (err, decodedToken) => {
			...
			if (err) {
				...
				const subject =
					'You Joined the Class Sucessfully ' + classes.name;
				const body = `
                        Dear ${student.name},
...
exports.showClass = async (req, res) => {
	try {
		...
		const userClass = await Class.findById(classId).populate(
			'faculty',
			'_id name'
		);
		const arrDiscussionsDetails = await Discussion.findOne({
			classId: classId,
		}).populate('discussion.sender_id');
...
exports.showClasswork = async (req, res) => {
	try {
		...
		arrAssignments.map(
			assignment =>
				(assignment.assignDate = dateFormator.format(
...
exports.showAssignment = async (req, res) => {
	try {
		...
		const studentMarks = assignment.homework.find(
			ww => ww.studentId.toString() == userId.toString()
		);

		console.log({ studentMarks });

		let user = await User.findById(userId);
		let isSubmitted = user.assignments.find(
			assign => assign._id == assignmentId
		)
			? user.assignments.find(assign => assign._id == assignmentId)
					.submitted
			: undefined;
...
exports.submitAssignment = async (req, res, next) => {
	try {
		...
		file.filename = `Classes/${userClass.name}_${classId}/Assignments/${
			assignment.title
		}_${assignmentId}}/Submissions/${
...
exports.POSTDiscussion = async (req, res) => {
	try {
		...
		if (file) {
			file.filename = `Classes/${userClass.name}_${classId}/Discussions/${
				file.originalname.split('.')[0]
			}_${Date.now()}.${file.mimetype.split('/')[1]}`;
