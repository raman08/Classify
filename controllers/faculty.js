...
exports.GETcreateAssignment = async (req, res) => {
	try {
		...
		if (user.role == 'student' || ind == -1) res.send('invalid');
			...
			res.render('Faculty/classes/create_assignment', {
				clss,
			});
	} catch (err) {
		console.log(err);
...
exports.POSTcreateAssignment = async (req, res, next) => {
	try {
		...
		file.filename = `Classes/${userClass.name}_${classId}/Assignments/${
			assignment.title
		}_${assignment._id}}/${file.originalname.split('.')[0]}_${Date.now()}.${
...
exports.updateMarks = async (req, res, next) => {
	try {
		...
		const url =
			'/faculty/show-class/' + classId + '/classwork/' + assignmentId;
...
exports.reportStudent = async (req, res) => {
	try {
		...
		let instituteDetails = await Inst.findById(student.inst).populate(
			'admin'
		);
