...
exports.getDashboard = async (req, res) => {
	try {
		const cls = await Inst.findOne({ _id: req.user.inst }).populate({
			path: 'classes',
...
exports.inviteStudentToClass = async (req, res) => {
	try {
		const { classId } = req.params;
		const cl = await Class.findById(classId).populate(
			'faculty',
			'_id name'
		); //classname
		const inst = cl.inst;
	...
	try {
		...
		for (let i = 0; i < arrClassDetailsPromise.length; i++) {
			const indexOfStudent =
				arrClassDetailsPromise[i].students.indexOf(memberId);
